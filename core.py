...
from __future__ import annotations # imported annotations module
from collections.abc import Iterable # imported Iterable module from collections.abc
from ctypes import CDLL # imported CDLL from ctypes
from datetime import datetime # imported datetime from datetime
from enum import IntEnum # imported IntEnum from enum
import io # import io module
import json # imported json module
from logging import Logger # imported Logger 
from math import ceil, isclose # imported ceil and isclose from math
import ntpath # imported ntpath
import os # imported os
import pathlib # imported pathlib
from queue import Queue # imported Queue from queue
from queue import Full, Empty
import re
import signal # imported signal
import sys # imported sys
from threading import Lock, Thread, Event # imported Lock, Thread and event from threading
from threading import current_thread, main_thread # imported current_thread, main_thread from threading
import time # imported time
from typing import Union, List, Optional, Dict, TypeVar, Any, Tuple, Callable
from urllib.parse import urlparse # imported ulparse from urllib.parse
from warnings import warn, simplefilter # imported warn, simplefilter from warnings
import weakref # imported weakref
...
from genicam.genapi import register, deregister, ECallbackType # imported register , deregsiter from genicam.genapi
from genicam.genapi import GenericException as GenApi_GenericException
from genicam.genapi import LogicalErrorException # imported LogicalErrorExcepotion from genicam.genapi
...
_sleep_default = 0.00001  # define default sleep value 
...
class ParameterSet:
    ...
    configure a subject.
    """
    def __init__(self, # intiliazing the parameterSet class
...
class ImageAcquirer:
    ...
    def start_acquisition(self, run_in_background: bool = False) -> None: # aquisition started 
        """
        Attention
        ---------
        ...
        self.start(run_as_thread=run_in_background) # start method is invoked
        
###############START IMAGE ACQUISITION ###############(TRIGGER _CAMERA)
            ...
            try:
                ...
                if acq_mode == 'Continuous':
                    ...
                    num_images_to_acquire = \
                        self.remote_device.node_map.AcquisitionFrameCount.value # image Acquisition happens
                else:
                    num_images_to_acquire = -1
...
class Harvester:
    ...
    def create(
        ...
        def compose_message(status: int, solution: int) -> str:
            ...
            solution_dict = {
                ...
                4: "Device Found but only for demo connection"
            }
        ...
        if type(search_key) is int:
            ...
            if num_candidates > 1:
                ...
                raise ValueError(compose_message(4, 4)) 
            else:
    ...
    def add_file(
            self, file_path: str, check_existence: bool = True,
